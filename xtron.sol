...
contract  XTron{
    ...
    uint256 constant public MAX_DAILY_WITHDRAWAL = 25000 trx;
    uint256 constant public MAX_DAILY_Ref_WITHDRAWAL = 10000 trx;
	uint256 constant public INVEST_MIN_AMOUNT = 100 trx;
	uint256[5] public REFERRAL_PERCENTS = [100, 50, 10, 10, 30];
	uint256[5] public REINVEST_REFERRAL_PERCENTS = [10, 4, 1, 1, 4];
	uint256 constant public MARKETING_FEE = 50;
	uint256 constant public SUPPORTING_FEE = 25;
	uint256 constant public ADMIN_FEE = 30;
	uint256 constant public DEV_FEE = 25;
	...
	function withdraw(uint _plan) public {
		...
		if(_plan != 2 && user.lastWithdrawn > 400 trx){
		...
		if(dividends > 0){
		    require(totalDividends >= 200 trx, "Min withdrawal is 200 trx");
		}
